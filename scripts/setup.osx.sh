...
command -v git flow >/dev/null 2>&1 && echo "Already installed :)!" || brew install git-flow
...
command -v yarn >/dev/null 2>&1 && echo "Already installed" || brew install yarn --ignore-dependencies
